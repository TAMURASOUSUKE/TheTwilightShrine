...
Boss::Boss(const Transform& _transform) : transform{ _transform }, clipIndex{ BOSS_START_ANIMATION_INDEX }
{
#ifdef _DEBUG
	handle = Gfx::LoadModel("Res/3DModel/Enemy/Boss.glb");
...
void Boss::Draw()
{
	...
	std::string playerClipIndexStr{ std::format("BossClipIndex : {}", clipIndex) };
	Gfx::DrawString(playerClipIndexStr.c_str(), { 0.0f, 60.0f });
#endif // _DEBUG
}